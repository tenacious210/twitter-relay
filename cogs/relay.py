...
class Relay(commands.Cog):
    def __init__(self, client):
        self.client = client
        with open('cogs/channels.json', 'r') as channels_json:
            self.channels = json.load(channels_json)
    ...
    async def init(self, ctx):
        ...
        with open('channels.json', 'w') as channels_json:
          json.dump(self.channels, channels_json)
        await ctx.send()
    ...
    def parsing(self):
        ...
        for ids in self.channels:
            ...
            self.client.loop.create_task(channel.send(f'{relayed_name} ({relayed_handle}): {relayed_tweet}'))
